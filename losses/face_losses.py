...
def arcface_loss(embedding, labels, out_num, w_init=None, s=64., m=0.5):
    """
    :param embedding: the input embedding vectors [batch_size, 128]
    ...
    with tf.variable_scope('arcface_loss'):
        ...
        inference_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=labels))

    return inference_loss, logits
...
def cos_loss(x, y, num_cls, reuse=False, alpha=0.35, scale=64, name='cos_loss'):
    ...
    xs = x.get_shape()
    with tf.variable_scope('centers_var', reuse=reuse):
        w = tf.get_variable("centers", [xs[1], num_cls], dtype=tf.float32,
                            initializer=tf.contrib.layers.xavier_initializer(), trainable=True)

    # normalize the feature and weight
    # (N,D)
    x_feat_norm = tf.nn.l2_normalize(x, 1, 1e-10)  # 按行进行规范化，也即是一行就是一个128D人脸特征，一共有batchSize行， 规范化的目的就是为了惩罚较大权重
    # (D,C)
    w_feat_norm = tf.nn.l2_normalize(w, 0, 1e-10)  # 按列进行规范化，也即是一列就是一个128D人脸特征，一共有人脸总数列
...
def softmax_loss(prelogits, labels, out_num):
    logits = tf.contrib.slim.fully_connected(prelogits, out_num, activation_fn=None)
    softmax_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=logits))
    return softmax_loss, logits